...
| 功能 | 说明 |
| --- | --- |
| **`@` 引用文件** | 输入框输入 `@` 弹出文件菜单：**最近引用置顶** + 当前目录文件/目录 + **斜杠层级导航**（`@src/components/` 直达子目录）；选中后插入真正的 **文件 chip**（浅蓝单框，显示 `@相对路径`，过长省略，**hover 查看完整路径**，整体删除/复制），提交时序列化为 `@相对路径` 随消息发送，agent 可直接 `read` |
